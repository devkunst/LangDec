...
class GeneralPRM(BasePRM):
    ...
    def call_with_kv_cache(
        ...
        for i in range(0, new_input_ids.shape[0], self.batch_size):
            ...
            output_scores.append(
                self.compute_scores(new_input_ids[i : i + self.batch_size], logits, return_all_steps=return_all_steps)
            )
        ...
        return scores, logits, (full_input_ids, past_key_values)
    ...
    def call_without_kv_cache(
        ...
        for i in range(0, full_position_ids.shape[0], self.batch_size):
            ...
            logits = outputs.logits[:, :, self.candidate_ids].to(self.secondary_device)
            output_scores.append(
                self.compute_scores(full_input_ids[i : i + self.batch_size], logits, return_all_steps=return_all_steps)
            )
        ...
        return scores, logits, (full_input_ids, None)
    ...
    def __call__(
        ...
        if self.use_past_key_values:
            scores, logits, new_state = self.call_with_kv_cache(new_input_ids, state, return_all_steps=return_all_steps)

        else:
            scores, logits, new_state = self.call_without_kv_cache(new_input_ids, state, return_all_steps=return_all_steps)
        ...
        return scores, logits, new_state
